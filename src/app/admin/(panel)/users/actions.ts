 ...
 *
 * Uses the native MongoDB driver to correctly query the `isBlocked` field
 * even on legacy documents where the field is absent (treated as falsy = active).
 */
export async function fetchUsersPage(
  ...
  // Treat documents where isBlocked is missing (undefined/null) as "active"
  const query: Record<string, unknown> = {};
  if (filter === "blocked") {
    query.isBlocked = true;
  } else if (filter === "active") {
    // Match documents where isBlocked is false OR the field doesn't exist
    query.$or = [{ isBlocked: false }, { isBlocked: { $exists: false } }];
  }
...
/**
 * Block or unblock a photographer's access.
...
export async function updateUserBlock(userId: string, isBlocked: boolean) {
  ...
  return {
    success: true,
    isBlocked: (result.isBlocked as boolean | undefined) ?? false,
  };
}
