...
export default function Navbar() {
  ...
  const navItems = [
    { href: eventBasePath ? `${eventBasePath}/live` : "/live", label: "Live Feed" },
    { href: eventBasePath ? `${eventBasePath}/gallery` : "/gallery", label: "Gallery" },
    { href: eventBasePath ? `${eventBasePath}/studio` : "/studio", label: "Studio" },
  ];

  const isActive = (path) => pathname === path;
  const homeHref = eventBasePath ? `${eventBasePath}/live` : "/";
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`cursor-pointer ${
              isActive(item.href) ? "text-white font-semibold" : "hover:text-white"
            }`}
          >
            {item.label}
          </Link>
